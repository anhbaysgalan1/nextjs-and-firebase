...
export class Actions {
  ...
  static updateDraftMessage(message) {
    return { type: 'UPDATE_DRAFT_MESSAGE', message };
  }

  // Thunks
  // ------

  static removeMessageForUser(uid, id) {
    return () => {
      firebase.database().ref(`messages/${uid}/${id}`).remove();
    };
  }
  ...
  static loadMessagesForUser(uid, firebaseInstance = firebase) {
    return dispatch =>
      new Promise(async (resolve) => {
        const messages = await firebaseInstance.database().ref(`messages/${uid}`).once('value');
  ...
  static addMessageToQueue(message) {
    return dispatch =>
      new Promise(async (resolve) => {
        ...
        firebase.database().ref(`messages/${message.user.uid}/${date}`).set({
          id: date,
          text: message.text,
...
const reducer = (
  ...
  switch (action.type) {
    ...
    case 'CLEAR_DRAFT_MESSAGE':
      return { ...state, draftMessage: Object.assign({}, defaultDraftMessage) };
    default:
      return state;
  }
};
